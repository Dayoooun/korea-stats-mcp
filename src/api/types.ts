...
// 통계설명 API 응답 (statisticsExplData.do, method=getList)
export interface StatisticsExplainItem {
  statsNm?: string;
  josaItm?: string;
  goalPoplExmnPopl?: string;
  estEqua?: string;
  examinObjrange?: string;
  examinObjArea?: string;
...
export interface SimplifiedDataItem {
  ...
  classificationCodes: Record<string, string | undefined>;
  item: string;
  itemId?: string;
  unit: string;
  unitId?: string;
  raw: StatisticsDataItem;
}
