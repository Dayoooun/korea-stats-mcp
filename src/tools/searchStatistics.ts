...
export const searchStatisticsSchema = {
  ...
  inputSchema: z.object({
    ...
    orgId: z.string().optional().describe("기관코드 (선택, 예: 101=통계청)"),
    sort: z
      .enum(["RANK", "DATE"])
      .optional()
      .default("RANK")
...
export type SearchStatisticsInput = z.infer<
  typeof searchStatisticsSchema.inputSchema
>;

export async function searchStatistics(input: SearchStatisticsInput): Promise<{
  success: boolean;
  totalCount: number;
  ...
  const searchTerms = generateSearchTerms(parsed);

  // 지역명 추출 (결과 필터링용)
  const regionNames = parsed.regions
    .map((code) => REGION_NAMES[code])
    .filter(Boolean);
  ...
  try {
    ...
    const hasPriorityTable = priorityTableIds.some((id) =>
      existingTableIds.has(id),
    );
    ...
    if (!hasPriorityTable) {
      ...
      if (fallbackTables.length > 0) {
        ...
        const newTables = fallbackTables.filter(
          (t) => !existingTableIds.has(t.TBL_ID),
        );
    ...
    if (results.length > 0) {
      ...
      results = results.sort((a, b) => {
        ...
        if (hasRegion) {
          const aHasRegion = regionNames.some(
            (region) =>
              (a.TBL_NM || "").includes(region) ||
              (a.STAT_NM || "").includes(region),
          );
          const bHasRegion = regionNames.some(
            (region) =>
    ...
    const simplifiedResults: SimplifiedStatisticsItem[] = limitedResults.map(
      (item) => ({
        orgId: item.ORG_ID,
        orgName: item.ORG_NM,
        tableId: item.TBL_ID,
