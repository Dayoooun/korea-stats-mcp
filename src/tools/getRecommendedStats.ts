 ...
 * - 검색 결과보다 우선 표시
 */
const PRIORITY_TABLES: Record<
  string,
  Array<{ orgId: string; tableId: string; name: string }>
> = {
  population: [
    {
      orgId: "101",
      tableId: "DT_1B04005",
      name: "시도/성/연령별 주민등록인구",
    },
  ...
  transport: [{ orgId: "101", tableId: "DT_1YL12001", name: "자동차등록현황" }],
  social: [{ orgId: "101", tableId: "DT_1YL13001", name: "범죄발생 현황" }],
};

// 분야별 추천 검색어 및 대표 통계
const TOPIC_RECOMMENDATIONS: Record<
  string,
  {
    name: string;
    description: string;
...
export const getRecommendedStatsSchema = {
  ...
  description: "관심 분야에 맞는 주요 통계표를 추천합니다.",
...
export type GetRecommendedStatsInput = z.infer<
  typeof getRecommendedStatsSchema.inputSchema
>;
