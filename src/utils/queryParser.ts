...
// 키워드 → 통계 분류 매핑
export const TOPIC_MAPPINGS: Record<
  string,
  {
    category: string;
    keywords: string[];
...
> = {
  ...
  출생: {
    category: "population",
    keywords: ["출생아수", "출생률", "합계출산율"],
  },
  사망: {
    category: "population",
    keywords: ["사망자수", "사망률", "기대수명"],
  },
  고령화: {
    category: "population",
    keywords: ["고령인구", "노인인구", "고령화율"],
  },
  혼인: { category: "population", keywords: ["혼인건수", "혼인율", "결혼"] },
  이혼: { category: "population", keywords: ["이혼건수", "이혼율"] },
  출산: {
    category: "population",
    keywords: ["출산율", "합계출산율", "저출산"],
  },
  ...
  근로: {
    category: "employment",
    keywords: ["근로시간", "노동시간", "근무시간"],
  },
  청년: {
    category: "employment",
    keywords: ["청년실업", "청년고용", "청년취업"],
  },
...
export function parseQuery(query: string): ParsedQuery {
  ...
  if (
    ...
    lowerQuery.includes("변화") ||
    lowerQuery.includes("추이")
  ) {
    result.intent = "trend";
  } else if (
    lowerQuery.includes("뭐") ||
  ...
  if (result.keywords.length === 0) {
    result.keywords = query.split(/\s+/).filter((word) => word.length > 1);
  }
 ...
 */
export function generateSearchTerms(parsed: ParsedQuery): string[] {
  const terms: string[] = [];
